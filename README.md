...
1. **先文档后代码——但只有"语义"在代码之前**：**语义变更**（字段 / 接口 / 状态机 / 权限标识 / 应用落点）先在**契约正文（§1–§8）**定下来，代码照着实现；实施完成后回写的是**事实登记**（契约 §9 变更记录、含生产 SQL 时 §10、接口.md 各应用接入状态）——**两者方向相反、不可互相替代**。例外的"代码在前"只有两种：代码是既成事实（老模块无契约 / 体系外已改）走模式 B 对账、旧项目接入初始化时从代码提炼标杆契约。需求模糊时先 `/module-explore` 探路（探索不是必经阶段，明确时直接建文档）；
