...
| `/module-plan` | `/module-plan {模块名}` / `{计划路径}` / `{模块名} 归档` | 实施计划的创建、修订、状态流转、**归档**（已完成/已废弃移入 `计划/archive/`）；**跨模块计划前校验所引用契约已存在** | 不实施代码；不改契约文档 |
...
| `/module-review` | `/module-review {模块名}` | 对照四件套与各应用代码审查：分级问题清单 + **多应用对账** + **完成度对账** + **测试覆盖完整性**（黑盒必备/白盒按规则）+ 回写查账 | 不改代码，只出清单与修复建议 |
...
| 场景 | 流程 |
|------|------|
| 正向开发 | `/module-doc {模块名}`（模式 A 契约先行，可停在契约）→ `/module-plan`（创建计划）→ `/module-code`（实施 + 回写）→ `/module-doc {模块名}`（收尾同步/对账）→ `/module-review`（验证） |
...
③ 日常开发   → （可选）/module-explore 探路 → /module-doc 定义与对齐（三模式）→ /module-plan 计划 → /module-code 执行+回写 → /module-review 验证
```
...
flowchart TB
    ...
    AB --> T
    AA -->|"无"| AC["完成：回写 + 计划标记已完成 + 提示 /module-doc 更新索引"]
```
...
临时项目里跑 11 项核心回归：官方计划模板渲染后 `check` 通过（文档路径/SQL/标题行不误报）、代码围栏示例不进白名单、含空格路径与 `./` 前缀解析、`diff-check` 对「本次不改」应用的越界拦截、坏计划显式报出、目录参数干净报错、英文模式（`doc-framework-en/`）与旧英文目录兼容、探索/ 豁免、v1.x 向后兼容。
