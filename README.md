...
| `/module-code` | `/module-code {计划路径}` / `{模块名}` | **只执行「已批准」的实施计划**，以计划为主、契约为约束、**白名单为边界**（只允许改动计划「逐应用表态=改动」的应用代码根 ∩ 变更文件清单），按依赖序实施，完成后回写 | 不创建 / 修改四件套文档；不创建 / 修订计划 |
| `/module-review` | `/module-review {模块名}` | 对照四件套与各应用代码审查：分级问题清单 + **多应用对账**（落点 ↔ 表态 ↔ 接入状态 ↔ git diff）+ 回写查账 | 不改代码，只出清单与修复建议 |

### 1. /module-doc：模块文档（四件套）

**四件套**聚合在 `模块/{模块名}/` 下：`契约.md`（语义约束，10 章含接口概览与 mermaid 图；**主责服务声明 + §5 应用落点表**；§10 生产环境 SQL 执行登记防上线遗漏）、`接口.md`（字段级**服务端真相**，唯一事实源；含服务归属/消费方与各应用接入状态）、`测试.md`、`test.sh`。
...
└── 模块/{模块名}/       ← 模块聚合资产（业务语义唯一真相，应用只是落点）
    ├── 契约.md          ← 语义约束（10 章：主责服务声明 + §5 应用落点表 + §10 生产 SQL 登记，mermaid 图）
    ├── 接口.md          ← 字段级服务端真相（唯一事实源）：服务归属/类型/消费方 + 各应用接入状态
    ├── 测试.md
    ├── test.sh          ← 多服务 BASE_URL 参数化
    └── 计划/            ← 单模块实施计划（YYYY-MM-DD-{主题}.md：逐应用表态+按应用分节+依赖序）
```
...
`profile.md.tpl`（项目档案，含应用清单）· `接入指南.md.tpl` · `AGENTS.md.tpl` · `总契约.md.tpl` · `测试规范.md.tpl` · `接口规范.md.tpl` · `规范/类型-前端.md.tpl`、`规范/类型-后端.md.tpl`、`规范/应用-{应用标识}.md.tpl`（规范三层） · `边界/{能力}边界.md.tpl` · `模块/{模块名}/{契约|接口|测试|test.sh}.tpl` · `计划/YYYY-MM-DD-{实施主题}.md.tpl`
...
校验文档体系完整性（退出码 0=通过 / 1=有问题）：① **骨架完整性**——必需文件/目录是否存在（项目档案、总契约、测试/接口规范、模块/边界/规范/计划）；② **应用清单健康**——档案含「应用清单」时逐应用校验（规范文件存在、代码根存在且不嵌套、按应用类型要求类型层规范；旧版档案无应用清单则回退校验前后端规范）；③ **占位符残留**——扫描文档根下 .md 中未渲染的 `{占位符}`（只报含中文的占位符，避免误报 API 路径参数 `/{id}`、MyBatis `#{version}` 等英文花括号）；④ **引导清理**——接入指南.md、骨架引导 README、AGENTS.md 接入引导段是否已移除。文档根自动识别中英文模式（`doc-framework/` 或 `docs-framework/`）。初始化与日常维护后均可运行。
