...
> **中文命令说明**：`/文档` `/代码` `/检查` 是方法论约定（跨 agent 记忆点，Claude Code 等可按各自约定使用），但 **DSH 的显式触发只匹配小写字母/数字/连字符的英文技能名**（`/技能名` 手势），中文命令无法触发。也可用自然语言描述（如"用模块代码技能…"），AI 会按技能目录自行加载。
...
flowchart TB
    subgraph G["日常开发循环"]
        ...
        I --> J["/module-doc：自动探测模块状态<br/>（模块文件夹 / 契约 / 代码存在性）"]
        ...
        L --> O["开发者确认文档"]
        M --> P["用户确认草稿 / 修正"]
        N --> Q["/module-code：读上下文<br/>（契约 = 约束，标杆，边界）"]
        O --> Q
        P --> Q
        ...
        U --> V{"有偏差？"}
        V -->|"是"| W["/module-doc 同步：更新接口.md / 测试<br/>（语义变化走 /module-doc 改造）"]
        W --> T
        ...
        Z --> AA{"发现问题？"}
        AA -->|"代码 Bug"| T
        AA -->|"契约缺口"| AB["先补契约（/module-doc），再改代码"]
        AB --> T
        AA -->|"无"| AC["完成：回写事实登记（契约变更记录 /<br/>测试执行记录）+ 提示 /module-doc 同步（接口.md / 索引）"]
    end
```
