...
- `doc-framework list`——列出**在途计划**（默认排除「已完成/已废弃」与归档目录），显示 模块 / 计划（带「（轻量）」「（待合并）」「（已合并）」与增量条数）/ 状态 / 涉及应用 / 任务进度；`--module <名>` 只列某模块、`--app <标识>` 只列会改动该应用的计划、`--state <状态>` 过滤、`--stale <天数>` 只列日期早于 N 天前的在途计划（**批量归档入口**）、`--orphans` 只列"缺契约且无在途计划"的模块（**代码是否存在需人工确认**）、`--all` 含已完成/已废弃、`--json` 机器可读；
