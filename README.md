...
> 文档语言默认中文，可整体切换英文——**语言由根目录名识别**：中文 `doc-framework/`、英文 `doc-framework-en/`。语言模式只决定根目录**内部**的文件/目录命名（`项目档案.md` vs `profile.md`、`模块/` vs `modules/`），且要么全中文、要么全英文，禁止混用。
...
> **英文模式的当前边界**：英文模式是**骨架级支持**——根目录名与骨架文件/目录按英文命名（`profile.md` / `contract.md` / `modules/` / `plans/` …），`check` 据此校验骨架；但**应用清单与计划的解析仍只认中文节名与 `模块/`、`计划/` 目录**，因此 `diff-check` / `list` / `show` 在英文模式下不生效（回退为「档案缺少应用清单」提示）。要让英文模式与中文模式能力对齐，需再定义一套英文节名映射，属独立改造。
...
临时项目里跑 16 项核心回归：官方计划模板渲染后 `check` 通过（文档路径/SQL/标题行不误报）、代码围栏示例不进白名单、含空格路径与 `./` 前缀解析、`diff-check` 对「本次不改」应用的越界拦截、坏计划显式报出、目录参数干净报错、英文模式（`doc-framework-en/`）与旧英文目录兼容、探索/ 豁免、v1.x 向后兼容、**直改通道对账**（落点内通过 / 落点外拦截）、**轻量计划形态**（合规通过 / 缺表态硬报错）与 `list --stale`。
