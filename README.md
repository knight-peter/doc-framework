...
{
  "dependencies": {
    "doc-framework": "git+https://github.com/knight-peter/doc-framework.git#v1.0.5"
  },
  "pnpm": {
...
建议钉版本：`git+...#v1.0.5`。不带 `#ref` 时解析的是默认分支（main）的**最新提交**，而非最新标签；且 lockfile 会把解析到的提交 SHA 锁死，之后 main 有新提交也不会自动更新，需要 `pnpm update doc-framework`（或删除 lockfile 重新 install）。
