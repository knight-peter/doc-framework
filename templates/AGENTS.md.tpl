...
- 编码纪律：**语义先于代码、记录后于代码**——契约正文（§1–§8：数据模型/接口/状态机/权限/应用落点）是代码的约束，**语义变更必须在实施前更新**（`/module-doc` 模式 A）；实施后回写的是**事实登记**（契约 §9 变更记录、含生产 SQL 时 §10、接口.md 各应用接入状态）。例外：代码是既成事实（老模块无契约 / 体系外已改）→ `/module-doc` 模式 B 从代码反向对账。测试发现问题先定性（代码 Bug vs 契约缺口），涉及约定变化必回写契约。
