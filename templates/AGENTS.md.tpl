...
- 实施计划文档：单模块 `{文档根}/模块/{模块名}/计划/YYYY-MM-DD-{主题}实施.md`；跨模块 `{文档根}/计划/YYYY-MM-DD-{主题}实施.md`（**文件名必须带日期前缀**，便于归档；契约文档不带日期，演进记录在"变更记录"章节）。计划状态（待审核/已批准等）由 /module-plan 维护，/module-code 只执行「已批准」计划；计划的「逐应用表态 + 变更文件清单」即 /module-code 的路径白名单。
